...
public class HistoricalAnalyzer {
    ...
    public Map<String, Date> analyzeRepositoryHistory(File repoDir, String manualLogFilePath, CheckpointManager checkpointManager) throws IOException, GitAPIException {
        return analyzeRepositoryHistory(repoDir, manualLogFilePath, checkpointManager, null);
    }

    public Map<String, Date> analyzeRepositoryHistory(File repoDir, String manualLogFilePath, CheckpointManager checkpointManager, String specificCommitHash) throws IOException, GitAPIException {
        ...
        try (PrintWriter manualLogWriter = new PrintWriter(new FileWriter(manualLogFilePath, true))) {
            ...
            try (Repository repository = builder.setGitDir(new File(repoDir, ".git"))
                ...
                // Jeśli mamy konkretny commit do przeanalizowania
                if (specificCommitHash != null && !specificCommitHash.isEmpty()) {
                    System.out.println("Analizuję tylko konkretny commit: " + specificCommitHash);
                    ObjectId commitId = repository.resolve(specificCommitHash);
                    if (commitId == null) {
                        System.err.println("Nie znaleziono commita o hashu: " + specificCommitHash);
                        return firstFeatureCommitDate;
                    }

                    RevCommit commit = walk.parseCommit(commitId);
                    List<RevCommit> singleCommitList = new ArrayList<>();
                    singleCommitList.add(commit);

                    // Analizuj pojedynczy commit
                    for (RevCommit singleCommit : singleCommitList) {
                        printCommitInfo(singleCommit, 1, 1);
                        strategies.forEach(SyntaxAnalyzerStrategy::reset);
                        Map<String, Integer> currentCommitFeatureOccurrences = new HashMap<>();
                        analyzeFilesInCommit(repository, singleCommit, manualLogWriter, currentCommitFeatureOccurrences, firstFeatureCommitDate, detectedFeaturesInHistory);
                    }

                    // Zapisz punkt kontrolny
                    checkpointManager.saveCheckpoint(specificCommitHash, true);
                    return firstFeatureCommitDate;
                }
                ...
                for (int i = startIndex; i < commitList.size(); i++) {
                    ...
                    analyzeFilesInCommit(repository, commit, manualLogWriter, currentCommitFeatureOccurrences, firstFeatureCommitDate, detectedFeaturesInHistory);
    ...
    private void analyzeFilesInCommit(Repository repository, RevCommit commit, PrintWriter manualLogWriter, Map<String, Integer> currentCommitFeatureOccurrences, Map<String, Date> firstFeatureCommitDate, Set<String> detectedFeaturesInHistory) throws IOException {
    ...
    // Dodaj nową metodę dla debugowania konkretnego commita
    public void debugSpecificCommit(File repoRoot, String commitHash, String manualLogFilePath) {
        System.out.println("=== DEBUG: Analizuję konkretny commit: " + commitHash + " ===");

        try (Repository repository = FileRepositoryBuilder.create(new File(repoRoot, ".git"));
             Git git = new Git(repository);
             RevWalk revWalk = new RevWalk(repository)) {

            ObjectId commitId = repository.resolve(commitHash);
            if (commitId == null) {
                System.err.println("Nie znaleziono commita: " + commitHash);
                return;
            }
            ...
            System.out.println("Data: " + dateFormat.format(commitDate));
            System.out.println("Wiadomość: " + commit.getFullMessage().trim());

            // Reset strategii
            strategies.forEach(SyntaxAnalyzerStrategy::reset);

            // Analizuj pliki w tym commicie
            RevTree tree = commit.getTree();
            try (TreeWalk treeWalk = new TreeWalk(repository)) {
                treeWalk.addTree(tree);
                treeWalk.setRecursive(true);
                        ...
                        try {
                            ...
                            for (JavaParser parser : javaParsers) {
                                try {
                                    ...
                                    // Ignoruj błędy parsowania i spróbuj następny parser
                                }
                            }

                            if (cu != null) {
                                analyzedFilesCount++;
                                Path javaFilePath = Path.of(filePath);

                                // Analizuj plik wszystkimi strategiami
                                for (SyntaxAnalyzerStrategy strategy : strategies) {
                                    int beforeCount = strategy.getTotalOccurrences();
                                    strategy.analyze(cu, javaFilePath, fileContent);
                                    int afterCount = strategy.getTotalOccurrences();

                                    if (afterCount > beforeCount) {
                                        System.out.println("ZNALEZIONO " + strategy.getName() + " w pliku: " + filePath +
                                                         " (+" + (afterCount - beforeCount) + " wystąpień)");
                                    }
                                }
                            }
                        } catch (Exception e) {
                            System.err.println("Błąd analizy pliku " + filePath + ": " + e.getMessage());
                        }
                    }
                }

                System.out.println("Przeanalizowano " + analyzedFilesCount + " z " + javaFilesCount + " plików Java");

                // Pokaż wyniki
                for (SyntaxAnalyzerStrategy strategy : strategies) {
                    if (strategy.getTotalOccurrences() > 0) {
                        System.out.println(strategy.getName() + ": " + strategy.getTotalOccurrences() + " wystąpień");

                        // Zapisz do CSV jeśli są wystąpienia
                        saveFeatureOccurrencesToCSV(strategy, commit, author, commitDate, manualLogFilePath);
                    }
                }
            ...
            System.err.println("Błąd podczas analizy commita " + commitHash + ": " + e.getMessage());
            e.printStackTrace();
        }
    }

    private void saveFeatureOccurrencesToCSV(SyntaxAnalyzerStrategy strategy, RevCommit commit, String author, Date commitDate, String manualLogFilePath) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(manualLogFilePath, true))) {
            for (SyntaxAnalyzerStrategy.FeatureOccurrence occurrence : strategy.getOccurrences()) {
                writer.printf("\"%s\";%s;%s;%s;%s;%d;\"%s\";%s%n",
                        author,
                        commit.getId().getName(),
                        dateFormat.format(commitDate),
                        strategy.getName(),
                        occurrence.getFilePath(),
                        occurrence.getLineNumber(),
                        occurrence.getLineContent().replace("\"", "\"\""),
